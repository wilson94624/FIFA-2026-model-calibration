...
Expected Goals，簡稱 xG，可以翻成「預期進球數」。在這個 repo 中，xG 不是從射門資料算出來，而是從 Elo 差距轉換出來。也就是說，我們用兩隊強弱差去估計兩隊平均會進幾球。
...
我們也發現 baseline_current 的平均總進球預測偏低。`results/final_worldcup_model_benchmark.json` 顯示 baseline_current 預測平均總進球是 2.560094，但實際是 2.725641。
...
我們也發現世界盃中立場需要自己的校準。全量國際賽最佳公式不一定直接等於 World Cup / Euro neutral 的最佳公式。
...
這個設定代表：我們不再把世界盃中立場當成一般主客場國際賽，而是用專門的中立場 xG 公式。
