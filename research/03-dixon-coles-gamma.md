...
但足球比分有一個麻煩：兩隊進球不一定完全獨立，而且低比分特別敏感。0-0、1-0、0-1、1-1 這些比分對勝平負機率有很大影響。因此我們研究 Dixon-Coles rho 和 Gamma。
...
我們想知道：這兩個修正是不是能用很小的改動改善 final candidate，而不是引入一個大型新模型。
...
我們固定 xG 候選後，掃過不同 rho 與 gamma，觀察：
...
rho=0.05 的好處是整體機率分數較好；rho=0.0 雖然 Top-1 比分較好，但我們不能只為了最高比分命中率犧牲 LogLoss。

Gamma 的結果也提醒我們：Brier Score 和 LogLoss 有時會選不同參數。這時我們更重視 LogLoss，因為它對過度自信的錯誤更敏感。
