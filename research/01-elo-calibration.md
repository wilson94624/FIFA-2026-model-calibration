...
Elo 是整個模型的地基。它把一支國家隊的歷史強弱壓成一個數字，分數越高代表模型認為隊伍越強。足球預測裡，我們先用 Elo 判斷兩隊強弱差，再把這個差距轉成勝平負機率與預期進球數。
...
另一個問題是國際賽資料很混雜：友誼賽、資格賽、洲際盃、世界盃，強度都不同。如果我們過度信任某些比賽的勝差，Elo 可能被少數大比分拉歪；但如果完全不看勝差，又會浪費資訊。
...
我們先不碰複雜球員資料，而是把這些基礎參數跑 benchmark，確認 Elo 本身能不能被校準好。
...
`results/time_split_shrinkage_validation.csv` 顯示，`calibrated_elo_v2_candidate` 的 LogLoss 最低，為 0.887639；但 `calibrated_elo_v3_shrinkage_alpha_0.4` 的 Accuracy 最高，為 0.598284。這提醒我們：最高 Accuracy 不一定代表最好的機率校準。
...
第二個發現是：勝差資訊有用，但要用溫和的形式。`log_margin` 比完全不用勝差更好，表示大勝提供資訊；但我們仍然要避免讓極端比分過度放大。
