...
from src.tuning.worldcup_xg_fine_search import (
    REPORT_COLUMNS,
    search_worldcup_xg_fine_parameters,
    write_outputs,
)
...
def test_search_worldcup_xg_fine_parameters(tmp_path: Path) -> None:
    ...
    rows, payload = search_worldcup_xg_fine_parameters(
        matches,
        universe,
        base_values=(1.35, 1.40),
        c1_values=(1.0,),
        scale_values=(500.0,),
    )

    assert len(rows) == 2
    assert rows[0]["world_cup_matches"] == 1
    assert rows[0]["euro_matches"] == 1
    assert rows[0]["pooled_matches"] == 2
    assert payload["universe"]["target_matches"] == 2
    assert "best_pooled_log_loss" in payload["summary"]


def test_write_worldcup_xg_fine_outputs(tmp_path: Path) -> None:
    matches = tmp_path / "matches.csv"
    universe = tmp_path / "universe.csv"
    ...
    assert "recommended_neutral_worldcup_xg_candidate" in parsed["summary"]


def test_worldcup_xg_fine_search_cli(tmp_path: Path) -> None:
    matches = tmp_path / "matches.csv"
    universe = tmp_path / "universe.csv"
    csv_path = tmp_path / "fine.csv"
    json_path = tmp_path / "fine.json"
    ...
    result = subprocess.run(
        [
            sys.executable,
            "src/tuning/worldcup_xg_fine_search.py",
            "--input",
            str(matches),
            "--team-universe",
            str(universe),
    ...
    assert "best_pooled_log_loss:" in result.stdout
